...
def make_transfer_agent(obs_shape, action_shape, args, device):
    if args.lts:
        ...
        return SacTransferImitateDetAgent(
                obs_shape=obs_shape,
                action_shape=action_shape,
                device=device,
                hidden_dim=args.hidden_dim,
...
def riro(imi_agent, sim_agent, replay_buffer, env, args):
    episode, episode_reward, done = 0, 0, True
    for step in range(args.num_post_rollout_steps):
        if done:
            switch_timestep = np.random.randint(200,500)
            obs, state = env.reset()
            done = False
            episode_reward = 0
            episode_step = 0
...
def main(args):
    ...
    riro_replay_buffer = utils.MultiStepReplayBuffer(
        obs_shape=env.observation_space.shape,
        state_shape=env.state_space.shape,
        action_shape=env.action_space.shape,
        ...
        if done:
            ...
            if not args.no_dac:
                ...
                if cost_function and episode % args.imitation_freq == 0:
                    ...
                    riro(imitation_agent, expert_agent, riro_replay_buffer, source_env, args)
