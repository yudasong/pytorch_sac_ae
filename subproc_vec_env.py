...
def _worker(
    ...
    while True:
        try:
            ...
            if cmd == "step":
                observation, state, reward, done, info = env.step(data)
                if done:
                    # save final observation where user can get it, then reset
                    info["terminal_observation"] = observation
                    observation, state = env.reset()
                remote.send((observation, state, reward, done, info))
            elif cmd == "seed":
...
class SubprocVecEnv(VecEnv):
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        return np.stack(state), np.stack(rews), np.stack(dones), infos
