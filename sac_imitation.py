...
class Actor(nn.Module):
    ...
    def log(self, L, step, log_freq=LOG_FREQ):
        ...
        for k, v in self.outputs.items():
            L.log_histogram('imitation_actor/%s_hist' % k, v, step)

        L.log_param('imitation_actor/fc1', self.trunk[0], step)
        L.log_param('imitation_actor/fc2', self.trunk[2], step)
        L.log_param('imitation_actor/fc3', self.trunk[4], step)
...
class Critic(nn.Module):
    ...
    def log(self, L, step, log_freq=LOG_FREQ):
        ...
        for k, v in self.outputs.items():
            L.log_histogram('imitation_critic/%s_hist' % k, v, step)

        for i in range(3):
            L.log_param('imitation_critic/q1_fc%d' % i, self.Q1.trunk[i * 2], step)
            L.log_param('imitation_critic/q2_fc%d' % i, self.Q2.trunk[i * 2], step)
...
class SacAeImitationAgent(object):
    """SAC+AE algorithm."""
    def __init__(
        self,
        obs_shape,
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, L, step):
        ...
        L.log('imitation_critic/loss', critic_loss, step)
    ...
    def update_actor_and_alpha(self, obs, L, step):
        ...
        L.log('imitation_actor/loss', actor_loss, step)
        L.log('imitation_actor/target_entropy', self.target_entropy, step)
        entropy = 0.5 * log_std.shape[1] * (1.0 + np.log(2 * np.pi)
                                            ) + log_std.sum(dim=-1)
        L.log('imitation_actor/entropy', entropy.mean(), step)
        ...
        L.log('imitation_alpha/loss', alpha_loss, step)
        L.log('imitation_alpha/value', self.alpha, step)
        alpha_loss.backward()
    ...
    def update_decoder(self, obs, target_obs, L, step):
        ...
        L.log('imitation_ae/ae_loss', loss, step)
    ...
    def update(self, replay_buffer, L, step):
        ...
        L.log('imitation/batch_reward', reward.mean(), step)
